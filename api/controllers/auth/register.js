...
module.exports = async function register(req, res) {
  ...
  try {
    if (req.method === 'GET') {
      return res.view('web/auth/register'); // views/pages/login.ejs
    }
    let post = req.body;
    ...
    `;
      ...
      delete(newUser.password);
      error = 'User created. Activation email sent to ' + post.email;
      return res.view('web/auth/emailsent', {
        email: post.email
      });
    }
  } catch (error) {
    error = 'Server error';
    return res.view('web/auth/register', {error:error}); 
  }
  return res.view('web/auth/register', {error:error}); 
};
